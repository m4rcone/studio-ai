...
interface Message {
  ...
  isStreaming: boolean;
  isError?: boolean;
  /** System-injected messages (preview transitions, approve/discard notifications).
   *  Excluded from the API history sent to Anthropic to avoid consecutive same-role errors. */
  isSystem?: boolean;
}
...
function saveMessages(branchName: string, messages: Message[]): void {
  try {
    // System messages (isSystem) are saved too — duplication on reload is
    // prevented by prevPreviewStatusRef being pre-seeded in initSession().
...
export function ChatInterface() {
  ...
  async function handleSend(text: string) {
    ...
    // Filter out system-injected messages to avoid consecutive same-role errors
    // (system messages have role "assistant" and could sit next to AI responses).
    ...
    await sendMessage(
      apiHistory,
      {
        ...
        onDone(newSession: EditSession | null) {
          ...
          setIsLoading(false);
          // Auto-focus the input so the user can immediately type the next message
        ...
        onError(error) {
          ...
          // If the stream timed out but tools executed (changes were saved),
          // fetch the latest session from the server to update the banner
          // and trigger preview notifications that the stream didn't deliver.
          if (error.includes("timed out")) {
            fetchOrRestoreSession().then((active) => {
              if (active && isMountedRef.current) setSession(active);
            });
          }
        },
      },
  ...
  function handleNewChat() {
    // If there's an active session with changes, confirm before discarding
    if (session && session.changes && session.changes.length > 0) {
      setShowNewChatModal(true);
      return;
    }
    cleanupAndStartNewChat();
  }

  async function handleNewChatConfirmed() {
    setNewChatLoading(true);
    try {
      await fetch("/api/studio/session/discard", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionSnapshot: sessionRef.current }),
      });
    } catch {
      // Best-effort — proceed with cleanup even if discard fails
    }
  ...
  function cleanupAndStartNewChat() {
    const branchName = sessionRef.current?.branchName;
    if (branchName) clearMessages(branchName);
    clearActiveBranch();
    clearSessionSnapshot();
  ...
  return (
    <>
      <ConfirmModal
        ...
        title="Discard changes and start over?"
        description={`You have ${session?.changes?.length ?? 0} pending change${(session?.changes?.length ?? 0) === 1 ? "" : "s"}. Starting a new chat will discard them. This cannot be undone.`}
        confirmLabel="Discard & start new"
        cancelLabel="Keep editing"
        destructive
        loading={newChatLoading}
        onConfirm={handleNewChatConfirmed}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          {isLoading && messages.every((m) => !m.isStreaming) && (
            <div className="mb-3 flex justify-start">
              ...
              <div className="bg-muted/60 rounded-(--radius) px-4 py-3">
